...
public class ClientProxy extends CommonProxy {
	...
	public void registerInformation() {
		...
		ClipboardStates save = NBTEdit.getClipboardSaves();
		save.load();
		save.save();
		...
		if (NBTEditKey.isPressed()) {
			...
			if (pos != null) {
				if (pos.entityHit != null) {
					NBTEdit.NETWORK.INSTANCE.sendToServer(new C2SEntityRequestPacket(pos.entityHit.getEntityId()));
				} else if (pos.typeOfHit == RayTraceResult.Type.BLOCK) {
					NBTEdit.NETWORK.INSTANCE.sendToServer(new C2STileRequestPacket(pos.getBlockPos()));
				} else {
